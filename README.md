...
<p align="center">
  ...
  Keep commercial pricing terms strictly confidential off-chain, while enforcing verifiable settlement on Coston2.
</p>
...
<p align="center">
  ...
  <a href="https://fcc.hushflow.dev/info"><strong>⚡ Simulated FCC Proxy</strong></a> ·
...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Settlement%20Drill-11%2F11%20Confirmed%20Txns-0F9D8A?style=for-the-badge" alt="11/11 Verified Coston2 Txns" />
  <img src="https://img.shields.io/badge/Flare%20FCC-SIMULATED__TEE%20Ready-8B5CF6?style=for-the-badge" alt="Flare FCC" />
...
2. **Liquidity Provider Desk (`/liquidity`):** Inspect active target RFQs and submit sealed bids backed by refundable USDT0 escrow.
3. **Portfolio & Claims (`/portfolio`):** View settled positions, claimable balances, and 100% collateral refunds for losing makers.
4. **Cryptographic Proof Center (`/proof`):** Inspect transaction proofs and direct explorer receipts on Flare Coston2 Testnet.
...
2. **Flare Confidential Compute (FCC):** A secure enclave runtime (`HUSHFLOW / RESOLVE_RFQ` adapter on official FCE scaffold) decrypts the private envelopes, selects the winning quote using deterministic tie-breaking rules, and signs an execution attestation.
3. **On-Chain Settlement:** The `HushFlowRfq` and `HushFlowResultVerifier` smart contracts verify the signed resolution on Flare Coston2, releasing the FXRP lot to the winner and USDT0 proceeds to the seller, while automatically refunding 100% of losing providers' collateral.

---

## 📍 Deployed Addresses & Live Endpoints (Coston2 Testnet - Chain 114)

| Component | Target / Address | Status / Link |
| :--- | :--- | :--- |
| **Live Web App** | `https://hushflow.vercel.app` | [Open Web App](https://hushflow.vercel.app) · `HTTP 200 OK` ✅ |
| **HushFlowRfq (Custody & Settlement)** | `0x5bdfb417953fd1f87383dc07b8677a5b9cc880ab` | [View on Coston2 Explorer](https://coston2-explorer.flare.network/address/0x5bdfb417953fd1f87383dc07b8677a5b9cc880ab) · `Bytecode Confirmed` ✅ |
| **FXRP Token (FTestXRP)** | `0x0b6A3645c240605887a5532109323A3E12273dc7` | [View on Coston2 Explorer](https://coston2-explorer.flare.network/address/0x0b6A3645c240605887a5532109323A3E12273dc7) · `Verified Token` ✅ |
| **USDT0 Collateral Token (USD₮0)** | `0xC1A5B41512496B80903D1f32d6dEa3a73212E71F` | [View on Coston2 Explorer](https://coston2-explorer.flare.network/address/0xC1A5B41512496B80903D1f32d6dEa3a73212E71F) · `Verified Token` ✅ |
...
| **Public Simulated FCC Proxy** | `https://fcc.hushflow.dev` | [Check Public `/info` Endpoint](https://fcc.hushflow.dev/info) · `HTTP 200 OK` ✅ |

---

## 🧾 Verifiable On-Chain Evidence Ledger (11/11 Coston2 Transactions)

HushFlow has verified and recorded the full multi-party RFQ settlement drill on Flare Coston2 Testnet across three distinct wallets (Seller, Provider A, Provider B):
...
## 🔍 Verifiability Status & Explicit Evidence Boundaries

We maintain strict technical honesty regarding what is live versus what is simulated:

| Verification Area | Verified Reality | Current Boundary |
| :--- | :--- | :--- |
| **Coston2 On-Chain Settlement** | 11/11 transactions confirmed and verifiable on explorer. Deployed contract bytecode confirmed. | On-chain settlement drill is completed and non-custodial pull claims work. |
| **Flare FCC Extension** | Official `fce-extension-scaffold` TypeScript adapter implemented and tested for `HUSHFLOW / RESOLVE_RFQ`. | Running in `SIMULATED_TEE=true` mode against Coston2 testnet. |
| **Public Proxy Endpoint** | Reachable at `fcc.hushflow.dev/info` via Cloudflare Tunnel. | Emits simulated enclave identity (`extensionId: 0xffff...`). Registered hardware `PRODUCTION` enclave attestation on FlareTeeManager is pending. |
| **Automated Test Evidence** | 325 Vitest TypeScript tests + 52 Foundry Solidity tests passing (100% green). | Comprehensive local unit, integration, and invariant coverage. |
...
```text
  ...
  └────────┬────────┘       └────────┬────────┘       └────────┬────────┘
   ...
   ════════════════════════════════════════════════════════════════════════
                     ...
                     Simulated TEE Enclave Runtime
     1. Ingests encrypted envelopes inside isolated execution memory
...
HushFlow natively integrates key Flare Network primitives:

1. **Flare Confidential Compute (FCC):** Off-chain privacy runtime where sealed multi-party negotiations occur without exposing order intent to the public mempool.
2. **Flare Time Series Oracle (FTSO V2):** Real-time decentralized price reference anchors for FXRP/USDT0 directly inside the trading cockpit and telemetry bars.
3. **Flare Coston2 / EVM:** Non-custodial on-chain asset custody and pull-based settlements.
...
4. **Comprehensive Test Suite:**
   ...
   - **52 Foundry Solidity Tests** (including invariant & fuzz testing on custody accounting and state transitions).
