 ...
 *
 * Vendored from flare-foundation/fce-extension-scaffold@e3f587949069780084e2ced8a53c9419ed05c250
...
export class Server {
  ...
  private async processAction(
    ...
    try {
      ...
      return [
        ...
        JSON.stringify({ error: `invalid action JSON: ${String(e)}` }),
      ];
    }
    if (!action || typeof action !== "object" || !action.data) {
      return [
        400,
        { "content-type": "application/json" },
        JSON.stringify({ error: "invalid action structure: missing data" }),
      ];
    }
    ...
    try {
      ...
      return [
        400,
        { "content-type": "application/json" },
        JSON.stringify({ error: `invalid hex in message: ${String(e)}` }),
      ];
    }
    ...
    if (!df || typeof df !== "object" || !df.opType) {
      return [
        400,
        { "content-type": "application/json" },
        JSON.stringify({ error: "invalid DataFixed: missing opType" }),
      ];
    }

    const handler = this.framework.lookup(df.opType, df.opCommand ?? "");
    if (!handler) {
      return [501, { "content-type": "text/plain" }, "unsupported op type"];
    }
  ...
  private async serve(
    ...
    send(res, status, headers, payload);
  }
}
